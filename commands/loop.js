...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (!queue || !queue.playing) return interaction.reply({ content: '⚠️ ไม่มีเพลงที่กำลังเล่น!', ephemeral: true }).catch(e => { })
        ...
        col.on('collect', async (button) => {
          ...
          switch (button.customId) {
            case 'queue':
              ...
              interaction?.editReply({ content: `✅ ลูปในคิว!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ เปิดใช้งานลูป!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ ปิดลูป!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ ปิดใช้งานลูป' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
