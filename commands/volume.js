...
module.exports = {
  name: "volume",
  description: "ปรับเสียงเพลง",
  permissions: "0x0000000000000800",
  options: [{
    name: 'volume',
    description: 'เลือกระดับเสียง',
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!queue || !queue.playing) {
        return interaction.reply({ content: '⚠️ ไม่มีเพลงที่กำลังเล่นที่!', ephemeral: true });
      }
      ...
      if (success) {
        ...
        return interaction.reply({ content: '❌ ปรับเสียงไม่ได้', ephemeral: true });
      }
    } catch (e) {
      console.error(e);
    }
